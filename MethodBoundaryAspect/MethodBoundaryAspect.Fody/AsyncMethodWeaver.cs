...
namespace MethodBoundaryAspect.Fody
{
    public class AsyncMethodWeaver : MethodWeaver
    {
            ...
            out InstructionBlock loadBuilder)
        {
            ...
            if (!(setException?.Operand is MethodReference setExceptionMethod))
            {
                // Логируем структуру handler'а для отладки
                var instructions = new List<string>();
                for (var inst = handler.HandlerStart; inst != handler.HandlerEnd && inst != null; inst = inst.Next)
                {
                    instructions.Add($"{inst.OpCode} {inst.Operand}");
                }
                var instructionsInfo = string.Join(" -> ", instructions);
                
                throw new InvalidOperationException($"[MethodBoundaryAspect] Could not find SetException method in handler for {_method.FullName}. " +
                    $"Handler instructions: {instructionsInfo}. This suggests UniTask exception handling structure is different.");
            }
            ...
            if (ldarg_0 == null || ldfldaBuilder == null)
            {
                throw new InvalidOperationException($"[MethodBoundaryAspect] Could not find expected instruction sequence before SetException in {_method.FullName}");
            }
        ...
        protected override void WeaveOnException(IList<AspectData> allAspects, Instruction instructionCallStart, Instruction instructionCallEnd, Instruction instructionAfterCall, IPersistable returnValue)
        {
            ...
            if (handler == null)
            {
                // Логируем детальную информацию о структуре exception handlers
                var handlerInfo = string.Join(", ", _moveNext.Body.ExceptionHandlers.Select(h => 
                    $"[Type: {h.CatchType?.FullName ?? "Any"}, Start: {h.HandlerStart?.OpCode}, End: {h.HandlerEnd?.OpCode}]"));
                
                // Логируем все поля state machine чтобы понять структуру
                var fieldsInfo = string.Join(", ", _moveNext.DeclaringType.Fields.Select(f => 
                    $"{f.Name}: {f.FieldType.FullName}"));
                
                System.Diagnostics.Debug.WriteLine($"[MethodBoundaryAspect] Could not find state machine catch block for {_method.FullName}");
                System.Diagnostics.Debug.WriteLine($"[MethodBoundaryAspect] Available exception handlers: {handlerInfo}");
                System.Diagnostics.Debug.WriteLine($"[MethodBoundaryAspect] State machine fields: {fieldsInfo}");
                
                // Пытаемся найти любой catch-all handler
                handler = _moveNext.Body.ExceptionHandlers.FirstOrDefault(h => h.CatchType == null || h.CatchType.FullName == "System.Exception");
                
                if (handler == null)
                {
                    throw new InvalidOperationException($"[MethodBoundaryAspect] Could not find any suitable exception handler for async method {_method.FullName}. " +
                        $"Available handlers: {handlerInfo}. This suggests UniTask state machine has different structure than expected. " +
                        $"State machine type: {_moveNext.DeclaringType.FullName}");
                }
                else
                {
                    System.Diagnostics.Debug.WriteLine($"[MethodBoundaryAspect] Using fallback catch-all handler for {_method.FullName}");
                }
            }
